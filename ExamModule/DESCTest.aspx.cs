...
{
    public partial class DESCTest : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                string query = "SELECT Answer FROM TBL_DESQAnswer WHERE Schedule_ID = " + sch + " AND Stud_ID = " + std + "";
                ...
                /*Statements and Query To retrieve And Display First Question*/
                SqlDataAdapter da1 = new SqlDataAdapter("SELECT * FROM TBL_DESQNA WHERE Schedule_ID='" + Session["Schedule"].ToString() + "'", Conn);
        ...
        protected void btn_next_Click(object sender, EventArgs e)
        {
            ...
            int sch = Convert.ToInt32(Session["Schedule"]);
            int std = Convert.ToInt32(Session["StudId"]);
            /* Checking If TextBox Is Empty Or Not, If Empty Skip */
            if (txt_ans.Text!=string.Empty)
            {
                /* If Not Empty Then Check If The Student Has Answered The Current Question Before */
                string query1 = "SELECT Answer FROM TBL_DESQAnswer WHERE Schedule_ID = " + sch + " AND Stud_ID = " + std + " AND Question_ID = " + qid + "";
                ...
                if (dt.Rows.Count == 0)
                {
                    ...
                    string insert_query = "insert into TBL_DESQAnswer (Schedule_ID,Stud_ID,Question_ID,Answer) values (" + sch + "," + std + "," + qid + ",'" + Convert.ToString(txt_ans.Text) + "')";
                ...
                else
                {
                    ...
                    string update_query = "update TBL_DESQAnswer set Answer = '" + Convert.ToString(txt_ans.Text) + "' where Schedule_ID = " + sch + " AND Stud_ID = " + std + " AND Question_ID = " + qid + "";
                    SqlCommand cmd = new SqlCommand(update_query, Conn);
                    cmd.ExecuteNonQuery();
                }
            }
            /*Empty The TextBox Again So That The PRevious Answer Does Not Carry On */
            txt_ans.Text = string.Empty;
            ...
            Session["i"] = i;
            string query = "SELECT Answer FROM TBL_DESQAnswer WHERE Schedule_ID = " + sch + " AND Stud_ID = " + std + " AND Question_ID = " + qid + "";
            SqlCommand comm = new SqlCommand(query, Conn);
            SqlDataReader DR1 = comm.ExecuteReader();
            if (DR1.Read())
            {
                txt_ans.Text = Convert.ToString(DR1.GetValue(0));
            }
            Conn.Close();
        }
        ...
        protected void btn_sub_Click(object sender, EventArgs e)
        {
            ...
            if (dt.Rows.Count == 0)
            {
                ...
                if (txt_ans.Text == string.Empty)
                {
                    string insert_query = "insert into TBL_DESQAnswer (Schedule_ID,Stud_ID,Question_ID,Answer) values (" + sch + "," + std + "," + qid + ",'" + "Default" + "')";
                    SqlCommand cmd = new SqlCommand(insert_query, Conn);
                    cmd.ExecuteNonQuery();
                }
                else
